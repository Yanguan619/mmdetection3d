...
> **报告导读**：
> - **§1 当前 demo 性能**：0.62s/帧（2026-09-18 当前代码，轨迹 / bev_pool A/B / view_transform 明细）
...
> 2026-09-18 + O3 热点消除（NonZero/Unique/Index）+ get_geometry numpy 副本预计算 + bev_pool 包装层快赢（当前 0.62s/帧）；
> 2026-09-21 demo.py 复用 `mmdet3d.apis.init_model` / `inference_multi_modality_detector`（与 multi_modality_demo 同通路），计时口径不变

## 一览

**当前 E2E：0.62 s/帧**（`[TIME] inference` 口径 = `model.test_step` 纯前向，
不含图像/点云加载与权重加载；warmup 2 轮后实测 5 轮，631-639ms；9/18 晚移除 x/y 交换后 613-618ms）。
...
优化轨迹：**57s 初版 → 1.1s（算子向量化）→ 0.7s（O3 热点消除）→ 0.66s（get_geometry numpy 副本预计算）→ 0.63s（bev_pool 包装层快赢）→ 0.62s（移除 x/y 交换）**，详见 §1.1。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>LiDAR 投影；点云 → 稀疏深度图 (1,6,1,256,704)</td>
      <td>matmul, indexing</td>
      <td>65 ms</td>
      <td>9%</td>
    </tr>
    <tr>
    ...
    <tr>
      <td rowspan="5"><b>view_transform</td>
      <td>dtransform</td>
    ...
    <tr>
      ...
      <td>CumSum, Scatter(唯一索引), Sort</td>
      <td>105 ms</td>
      <td>18%</td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="3"><b>合计（串行探针口径）</b></td>
      <td>—</td>
      <td><b>≈670 ms；E2E 实测 0.63 s</b></td>
      <td>100%</td>
    </tr>
  </tbody>
</table>

> **数据口径**（2026-09-18，当前代码，npu:7，E2E 0.63s/帧，48 dets）：稳态探针 n=3（各阶段
> 分别计时，synchronize 后取值）。**多数分段测量于 0.7s 代码状态**（bev_pool 已按 2026-09-18
> 包装层快赢更新：124→105ms）。
> 探针为**串行口径**（逐段 synchronize），分段合计 ≈670ms；E2E 实测 631-639ms
> （9/18 晚移除 x/y 交换后 613-618ms）
> （0.63s 代码状态）。**占比列按原 697ms 串行合计计**（快赢后逐段占比
> 略有偏移，未逐一重算）。Swin/FPN、
> dtransform/depthnet、detector 各行为探针整段测量后的按 profile 算子占比拆分（±5ms 精度）。

**一览表关键结论**：

1. **前三大耗时**：sparse_encoder 24%（~170ms）> Swin+FPN 23%（~158ms）> bev_pool 全链 18%（105ms）
2. **bev_pool 的真身**：AscendC kernel 只占 20ms，**包装层（cast/swap/contiguous）~48ms 仍是最大单一优化靶**（三段拆解见 §1.3）
3. **get_geometry CPU fp32 补丁**：NPU fp32 bmm 精度 bug (#13) 的代价，58ms CPU。
   numpy 副本预计算（现行默认）E2E 657ms、几何逐位一致；去补丁 664ms 但精度显著退化
   （barrier box 跳 3-6.3m）。三方 A/B 明细见 §1.3
4. **AICPU 残留 ~138ms/帧**：voxelize Sort 39 / 点云掩码 NonZero 28 / IndexPut 18 / ScatterElements 16
5. **数据搬运/布局风暴 ~270ms/帧（36%）**：GatherV2 66 + TransData 60 + Transpose 53 + Cast 48 + Index 47，

## 1. 当前 demo 性能（2026-09-18）
...
| get_geometry | 58 ms (CPU) | 三方 A/B 见下 |
| bev_pool | 105 ms | 前处理 27 + 包装 ~48 + kernel 20，见下 |
...
105 ms = 前处理 27 + AscendC 包装 ~48 + kernel 20。
...
> 当前状态：与文档 §1 一致（0.62-0.83s/帧，O3 全套优化已生效，bev_pool 走 AscendC）
