...
> **报告导读**：
> - **§1 当前 demo 性能**：0.63s/帧（2026-09-18 当前代码，轨迹 / bev_pool A/B / view_transform 明细）
...
> 2026-09-18 + O3 热点消除（NonZero/Unique/Index）+ get_geometry numpy 副本预计算 + bev_pool 包装层快赢（当前 0.63s/帧）

## 一览

**当前 E2E：0.63 s/帧**（`[TIME] inference` 口径 = `model.test_step` 纯前向，
不含图像/点云加载与权重加载；warmup 2 轮后实测 5 轮，631-639ms；9/18 晚移除 x/y 交换后 613-618ms）。

优化轨迹：**57s 初版 → 1.1s（算子向量化）→ 0.7s（O3 热点消除）→ 0.66s（get_geometry numpy 副本预计算）→ 0.63s（bev_pool 包装层快赢）**，详见 §1.1。
...
> （QuickCumsum 初版）** 快照。当前 demo 已优化到 0.63s/帧（§1），本节保留作为诊断记录。
...
> 采集命令：`python NPU/demo.py <args> --loop 8`（demo.py 单进程 8 轮推理）
