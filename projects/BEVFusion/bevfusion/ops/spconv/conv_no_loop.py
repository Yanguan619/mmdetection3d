...
class SparseConvolution(SparseModule):
    ...
    def _downsample_coords(self, indices: torch.Tensor):
        ...
        # 展开行：第 r 行重复 T[r] 次
        # torch.repeat_interleave(arange(Nk), T) 在 ONNX 导出时会生成 Loop 算子
        # （ONNX 无原生 repeat_interleave，动态 repeats 只能 fallback Loop，ATC 不支持）。
        # 用固定迭代次数的二分查找替代（Python for 常量迭代会被 TorchScript 展开为
        # 直线代码，不产生 Loop）：base[p] = r ⇔ starts[r] <= p < starts[r]+T[r]
        ...
        lo = torch.zeros(total, dtype=torch.long, device=device)
        hi = torch.full((total,), Nk, dtype=torch.long, device=device)
        for _ in range(24):
            mid = (lo + hi) // 2
            is_lo = starts[mid] <= pos
            lo = torch.where(is_lo, mid, lo)
            hi = torch.where(is_lo, hi, mid)
        base = lo
        rem = pos - starts[base]
